...
class Dtw:
    ...
    def calcDistance(self):
        self.mat_d = np.zeros( (len(self.seq_1), len(self.seq_2)), dtype=float )
        for i in range(len(self.mat_d)):
            for j in range(len(self.mat_d[i])):
                self.mat_d[i][j] = abs(self.seq_1[i] - self.seq_2[j])


    def calcGamma(self):

        self.mat_g = np.zeros( (len(self.seq_1), len(self.seq_2)) )
    ...
    def openFiles(self, argv):
        ...
        self.seq_1 = np.loadtxt(argv[0])
        self.seq_2 = np.loadtxt(argv[1])
